...
# run the script with nohup python3 splash_dataset_creator.py &
...
MODULES_TO_CMD = {"radixsort": "kernels/radix/./RADIX -p{threads} -n{problemsize}",
...
# parse results from time command
...
# Parsing results from perf command 
...
# Running each module and collecting data for each of them
# Each metric averaged over 5 runs
        ...
        for size in PROBLEM_SIZES[module_name]:
            ...
            for thread in threads:
                ...
                for metric in metrics:
                    metrics[metric] = metrics[metric] / NUM_RUNS

                metrics['single-thread-time'] = single_thread_time / NUM_RUNS
...
if __name__ == "__main__":
    ...
    for module in MODULES_TO_CMD:
        print(module)
        get_data_for_module(module, MODULES_TO_CMD[module])
